...
public class TimUtils {
    ...
    public static void copyAppDatabaseFiles(String packageName) {
        try {
            ...
            TimUtils.error(TAG, e.getLocalizedMessage());
        }
    }
     ...
     * Copy the application database files to external storage
     */
    public static void copyAppNormalFiles(String packageName) {
        try {
            File dbDirectory = new File("/data/data/" + packageName + "/files/");
            if (dbDirectory.exists()) {
                File[] dbFiles = dbDirectory.listFiles();
                for (int i = 0; i < dbFiles.length; i++) {
                    copyFileToExternalStorage(dbFiles[i]);
     ...
     */
	public static void copyAppPreferenceFiles(String packageName) {
		try {
